...
#include "iceberg/util/aligned_storage.hh"
...
ICEBERG_EXPORT void DieWithMessage(const std::string& msg);
ICEBERG_EXPORT void InvalidValueOrDie(const Status& st);
...
///
/// ```
///   iceberg::Result<Foo> result = CalculateFoo();
///   if (result.ok()) {
///     Foo foo = result.ValueOrDie();
///     foo.DoSomethingCool();
///   } else {
///     ICEBERG_LOG(ERROR) << result.status();
///  }
/// ```
///
...
///
/// ```
///   iceberg::Result<std::unique_ptr<Foo>> result = CalculateFoo();
///   if (result.ok()) {
...
///
/// ```
///   iceberg::Status CalculateFoo(int *output);
/// ```
///
/// This function may instead be written as:
///
/// ```
///   iceberg::Result<int> CalculateFoo();
/// ```
template <class T>
class [[nodiscard]] Result : public util::EqualityComparable<Result<T>> {
 ...
 public:
  ...
  Result(const Status& status) noexcept  // NOLINT(runtime/explicit)
      : status_(status) {
    if (ICEBERG_PREDICT_FALSE(status.ok())) {
  ...
  template <
      ...
      typename E = typename std::enable_if_t<
          ...
          !std::is_same_v<typename std::remove_reference_t<typename std::remove_cv_t<U>>,
                          Status>>>
  ...
  Result(const Result& other) noexcept : status_(other.status_) {
    if (ICEBERG_PREDICT_TRUE(status_.ok())) {
  ...
  /// \param other The value to copy from.
  template <typename U,
            typename E = typename std::enable_if_t<std::is_constructible_v<T, const U&> &&
  ...
  Result& operator=(const Result& other) noexcept {
    ...
    if (ICEBERG_PREDICT_FALSE(this == &other)) {
      return *this;
    }
    Destroy();
  ...
  template <typename U,
            typename E = typename std::enable_if_t<std::is_constructible_v<T, U&&> &&
                                                   std::is_convertible_v<U, T>>>
  Result(Result<U>&& other) noexcept {
    if (ICEBERG_PREDICT_TRUE(other.status_.ok())) {
  ...
  template <typename U,
            typename E = typename std::enable_if_t<std::is_constructible_v<U, T>>>
  Status Value(U* out) && {
    if (!ok()) {
      return status();
    }
  ...
  /// error.
  template <typename U,
            typename E = typename std::enable_if_t<std::is_constructible_v<U, const T&>>>
  Result<U> As() const& {
    if (!ok()) {
      return status();
    }
...
#define ICEBERG_ASSIGN_OR_RAISE_IMPL(result_name, lhs, rexpr)     \
  ...
  ICEBERG_RETURN_IF_(!(result_name).ok(), (result_name).status(), \
                     ICEBERG_STRINGIFY(rexpr));                   \
...
///   ICEBERG_ASSIGN_OR_RAISE(auto value, MaybeGetValue(arg));
///
/// Example: Assigning to an existing value:
///   ValueType value;
///   ICEBERG_ASSIGN_OR_RAISE(value, MaybeGetValue(arg));
///
/// WARNING: ICEBERG_ASSIGN_OR_RAISE expands into multiple statements;
/// it cannot be used in a single statement (e.g. as the body of an if
/// statement without {})!
///
/// WARNING: ICEBERG_ASSIGN_OR_RAISE `std::move`s its right operand. If you have
/// an lvalue Result which you *don't* want to move out of cast appropriately.
///
/// WARNING: ICEBERG_ASSIGN_OR_RAISE is not a single expression; it will not
/// maintain lifetimes of all temporaries in `rexpr` (e.g.
/// `ICEBERG_ASSIGN_OR_RAISE(auto x, MakeTemp().GetResultRef());`
