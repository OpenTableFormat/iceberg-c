...
function(check_boost_version source_dir expected_version)
  ...
  if(NOT EXISTS ${version_hpp})
    message(
      FATAL_ERROR
        "${version_hpp} not found. Please either \"rm -rf ${source_dir}\" "
  ...
  file(STRINGS "${version_hpp}" BOOST_VERSION_LINE REGEX "^#define[ \t]+BOOST_VERSION[ \t]+[0-9]+$")
  string(
    REGEX
    REPLACE "^#define[ \t]+BOOST_VERSION[ \t]+([0-9]+)$"
            "\\1"
            BOOST_VERSION
            "${BOOST_VERSION_LINE}")
  ...
  if(version VERSION_LESS expected_version)
    message(
      FATAL_ERROR
        "Boost v${version} in ${source_dir} is not new enough. "
...
function(do_build_boost root_dir version)
  ...
  set(configure_command ./bootstrap.sh --prefix=<INSTALL_DIR> --with-libraries=${boost_with_libs}
  ...
  set(build_command ${b2} headers stage ${boost_features})
  set(install_command ${b2} install)
  ...
  elseif(version VERSION_GREATER 1.79)
    ...
    string(
      ...
      " "
      boost_url
      https://mirrors.aliyun.com/blfs/conglomeration/boost/boost_${boost_version_underscore}.tar.bz2
...
set(Boost_context_DEPENDENCIES
    thread
    chrono
    system
    date_time)
...
macro(build_boost version)
  ...
  foreach(c ${Boost_BUILD_COMPONENTS})
    ...
    if((c MATCHES "coroutine|context") AND (WITH_BOOST_VALGRIND))
      set_target_properties(Boost::${c} PROPERTIES INTERFACE_COMPILE_DEFINITIONS "BOOST_USE_VALGRIND")
    endif()
  ...
  foreach(c ${Boost_BUILD_COMPONENTS})
    if(Boost_${c}_DEPENDENCIES)
      ...
      endforeach()
      set_target_properties(Boost::${c} PROPERTIES INTERFACE_LINK_LIBRARIES "${dependencies}")
      unset(dependencies)
    endif()
...
function(maybe_add_boost_dep target)
  ...
  if(NOT
     type
     MATCHES
     "OBJECT_LIBRARY|STATIC_LIBRARY|SHARED_LIBRARY|EXECUTABLE")
    return()
  endif()
